...
class NSGAIIAlgorithm():
  ...
  def measurecrowdingdistance(self):
        print(self.front)
        print("longueur:", len(self.front))
        for i in range(len(self.front)):
              self.front[i].sort(key = lambda x: x.fitnessValue1, reverse = True)
              self.front[i][0].crowdingdistance = float('inf')
              self.front[i][-1].crowdingdistance = float('inf')
              for j in range(1,len(self.front[i]) - 1):
                    self.front[i][j].crowdingdistance = (self.front[i][j - 1].fitnessValue1 - self.front[i][j + 1].fitnessValue1) / (self.idealpoint.fitnessValue1 - self.nadirpoint.fitnessValue1) + (self.front[i][j + 1].fitnessValue2 - self.front[i][j - 1].fitnessValue2) / (self.idealpoint.fitnessValue2 - self.nadirpoint.fitnessValue2)
